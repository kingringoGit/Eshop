...
class Signup(View):
    ...
    def validateCustomer(self,customer):
        error_message=None
        if (not customer.first_name): #agr first_name nhi lihe hai toh error batayega
            error_message="first name required !!"
        elif len(customer.first_name)<3:
            error_message="first name must be greter then 3"
        elif (not customer.last_name):
            error_message="last name is required"
        elif len(customer.last_name)<3:
            error_message="last name must br greater then 3"
        elif (not customer.phone):
            error_message="phone number must required"
        elif len(customer.phone)<10:
            error_message="phone number must be greater then 9"
